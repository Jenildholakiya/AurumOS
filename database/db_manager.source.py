...
class DBManager:
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                return [{
                    ...
                    "permissions": _json.loads(r["permissions"]) if isinstance(r["permissions"], str) else (r["permissions"] or [])
                } for r in rows]
        except Exception as e:
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                for (code, touch_key), required in requirements.items():
                    ...
                    rows = conn.execute(
                        "SELECT gr_wt, touch FROM stock_inventory WHERE TRIM(it_code)=? AND COALESCE(gr_wt,0)>0",
                        (code,)).fetchall()
                    if not rows:
                        errors.append(f"IT code {code} is not available in inventory.")
                        continue
                    if not any(abs(float(row['touch'] or 0) - touch) <= 0.01 for row in rows):
                    ...
                    if available + 0.001 < required:
                        errors.append(f"Stock over for IT code {code}: available {available:.3f}g, required {required:.3f}g.")
            ...
            with self._get_connection() as conn:
                rows = conn.execute("""
                    SELECT vch_id, customer, mobile, status, date, time_stamp,
                           COALESCE(ledger_fine, 0.0) AS ledger_fine,
                           CASE WHEN UPPER(status) IN ('PAID', 'UCHAK_PAID')
                                THEN COALESCE(total_amount, 0.0)
                                ELSE COALESCE(collected_fine, 0.0) END AS collected_fine,
                ...
                for row in rows:
                    ...
                    if same_name or same_mobile:
                        ...
                        if is_retail:
                            item['status'] = 'PAID'
                            item['collected_fine'] = float(item.get('total_amount') or 0.0)
    ...
    def is_setup_done(self) -> bool:
        """
        Returns True only if setup_done = '1' in app_config.
        Once setup is completed on a PC, it never asks again.
        Machine fingerprint is updated to current machine on each check.
        """
        try:
            ...
            _dblog(f"[SETUP] done={done} my_fp={fp[:8]}...")

            if not done:
                _dblog("[SETUP] setup_done != 1 -> show setup")
                return False

            # Update fingerprint to current machine (allows hardware changes without re-setup)
            stored_fp = rows.get('machine_fingerprint', '')
            if stored_fp != fp:
                _dblog("[SETUP] fingerprint changed -> updating to current machine")
                with self._get_connection() as conn:
                    conn.execute(
    ...
    def bastion_clear(self, admin_key, lock_code=None):
        ...
        Also accepts regular 12-char unlock keys (salt: AurumOS@Jewel#2024$Prof)
        from the web dashboard for backward compatibility.
        """
        import hashlib as _hl
        import json as _json
        from datetime import datetime as _dt, timedelta as _td
        ...
        REGULAR_SALT = 'AurumOS@Jewel#2024$Prof'  # Regular unlock salt
        ...
        try:
            ...
            # Try the stored lock code from suspension time first,
            # fall back to current machine fingerprint if not found.
            try:
                with self._get_connection() as conn:
                    row = conn.execute(
                        "SELECT value FROM app_config WHERE key='lock_code_cache'"
            ...
            # BASTION date candidates (IST only, +/- 3 days)
            bastion_dates = [
                (now_ist - _td(days=3)).strftime('%Y-%m-%d'),
                (now_ist - _td(days=2)).strftime('%Y-%m-%d'),
            ...
            # Regular unlock date candidates (IST, UTC, local, each +/- 1 day)
            regular_dates = []
            for base in [now_ist, now_utc, now_loc]:
                for delta in [0, -1, 1]:
                    ds = (base + _td(days=delta)).strftime('%Y-%m-%d')
            ...
            # First try BASTION salt (16-char key)
            for date_str in bastion_dates:
            ...
            # If no match and key is 12 chars, try REGULAR salt (12-char key from web dashboard)
            if not matched_date and len(entered) == 12:
                _dblog(f"[BASTION] Trying REGULAR salt for 12-char key, dates={regular_dates}")
                ...
                _dblog(f"[BASTION] Key length is {len(entered)}, not 12 - skipping REGULAR salt")

            _dblog(
                f"[BASTION] lc={lock_code} dates_tried={bastion_dates} entered={entered[:4]}**** matched={matched_date} salt={used_salt}")
            ...
            if matched_date:
                ...
                with self._get_connection() as conn:
                    ...
                    conn.commit()
                _dblog(f'[BASTION] Suspension cleared with {used_salt} salt on {matched_date}')
